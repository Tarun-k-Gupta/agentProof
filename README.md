...
> Project code lives in `agentProof/` — run all commands below from that directory (`cd agentProof`).
...
Every claim in this README that involves a deployed thing resolves to something below. Nothing here requires our servers, our permission, or our word for it: open a block explorer and read the bytecode, query the subgraph, resolve the ENS name and hash the policy yourself. `pnpm verify:deployment` (from `agentProof/`) re-checks the Sepolia addresses with `eth_getCode` against a public node.

The source of truth is `agentProof/deployments/*.json`, read directly by the SDK, the API and the dashboard. This table is a copy for readers; if the two ever disagree, the JSON is right.
...
The AgentProof contracts, then the third-party contracts they are pinned against. `verified` in `agentProof/deployments/sepolia.json` records which of these were confirmed to hold bytecode, when, and by what method.
...
That repoint pair is the only on-chain proof that owner-only repointing works. It is also honest about a limit: the hook could not follow, because a live `AgentPolicyHook` cannot be replaced. See `agentProof/docs/future-work.md`.
...
The test that makes this real is `agentProof/contracts/test/integration/RealAccount.t.sol`:
...
| Component | Path | Role |
|---|---|---|
| SDK (the product) | `agentProof/packages/sdk/` — zero runtime deps | Policy engine, 7 policies, decoders, ENS/The Graph/Hedera clients, simulated account for tests |
| Hook + library | `agentProof/contracts/src/AgentPolicyHook.sol`, `agentProof/contracts/src/lib/PolicyLib.sol` | On-chain enforcement of the critical subset |
| Identity | `agentProof/contracts/src/AgentSubnameRegistrar.sol`, `agentProof/packages/sdk/src/identity/` | ENSv2 subnames + hash-bound startup check |
| Verification API | `agentProof/packages/api/` — `node:http`, no framework | x402-gated `POST /v1/verify`, streams runs to the dashboard |
| Payment client | `agentProof/packages/x402-client/` | Pays for verification, policy-checks its own payment first (the loop closes) |
| Agents | `agentProof/apps/agents/trader`, `agentProof/apps/agents/researcher` | Demo consumers of the SDK |
| Dashboard | `agentProof/apps/dashboard/` (Next.js) | Read-only control plane + approval UI + proof drawer |
| Subgraph | `agentProof/subgraphs/agent-history/` | Indexes `SpendRecorded` events |
| Recipes | `agentProof/recipes/verify-before-you-swap.ts` | Gateway integration that imports nothing internal |

Two rules: `agentProof/deployments/*.json` is the single source of truth for addresses, and `agentProof/proofs/*.json` is generated by `pnpm verify:formal`, never hand-edited.
...
Full versions: `agentProof/docs/threat-model.md`, `agentProof/docs/future-work.md`, `agentProof/docs/privacy.md`, `agentProof/docs/formal-verification.md`.
...
Six, each load-bearing rather than decorative. What is *built* vs *qualified* (needs external accounts/hardware/hosting) is tracked per-integration in `agentProof/docs/future-work.md`.
...
- **Bazantic — verify before you swap.** `agentProof/recipes/verify-before-you-swap.ts` chains quote → `/v1/decode` → `/v1/verify` → execute-or-stop over plain HTTP, importing nothing internal. Failing to *obtain* a verification stops the swap like a BLOCK. See `agentProof/docs/integrations/bazantic.md`.

Privacy (implemented in `agentProof/packages/sdk/src/privacy/`, not promised here): evaluate locally (zero-dep in-process engine); log shapes not values (redaction on the way *in*); publish commitments not records (`keccak256(salt ‖ account ‖ intent ‖ decision)` on HCS); bucket public amounts (order-of-magnitude bands). See `agentProof/docs/privacy.md`.
...
Full version in `agentProof/docs/threat-model.md`.
...
| Claim | Method | Status |
|---|---|---|
| `MAX_TRANSFER`, `DAILY_SPEND` over `PolicyLib` | solc SMTChecker, CHC engine | see `agentProof/proofs/summary.json` (committed, with raw transcripts in `agentProof/artifacts/smt/`) |
...
Supporting commands: `pnpm verify:deployment` (bytecode + activity check over `agentProof/deployments/`), `pnpm capture:uniswap-fixtures` (refresh Sepolia decoder fixtures), `pnpm recipe:verify-before-you-swap`. See `agentProof/docs/quickstart.md` and `agentProof/docs/integrations/bazantic.md`.
...
Pinned versions that matter (see note at the foot of `agentProof/contracts/foundry.toml`): ERC-7579 reference account at v0.3.1, account-abstraction at **v0.8.0** (v0.7 predates `ISenderCreator`; v0.9 gates `handleOps` behind `tx.origin == msg.sender`).

---

## 14. Future work

`agentProof/docs/future-work.md` is the binding list. Headlines:
...
- [x] `pnpm verify:formal` regenerates `agentProof/proofs/*.json`; negative control yields a counterexample; raw transcripts in `agentProof/artifacts/smt/`.
- [x] `agentProof/deployments/*.json` is the single address source; `pnpm verify:deployment` checks bytecode + activity.
- [x] Threat model, formal-verification scope, privacy rules, and future-work gaps are written down in `agentProof/docs/` — no silent edges.
- [x] Hook + ERC-7579 account deployed on Sepolia (receipts in `agentProof/contracts/broadcast/`); `pnpm verify:deployment` checks bytecode + activity.
